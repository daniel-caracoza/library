...
class ObjectRenderer {
    ...
    //Shader location: object color property
    ...
    private val modelViewProjectionMatrixBuff = FloatBuffer.wrap(modelViewProjectionMatrix)
    ...
    fun draw(cameraView: FloatArray, cameraPerspective: FloatArray, colorCorrectionRgba: FloatArray, objColor: FloatArray){
        ShaderUtil.checkGLError(TAG, "Before draw")
        Matrix.multiplyMM(modelViewMatrix, 0, cameraView, 0, modelMatrix, 0)
        Matrix.multiplyMM(modelViewProjectionMatrix, 0, cameraPerspective, 0, modelViewMatrix, 0)
        ...
        Matrix.multiplyMV(viewLightDirection, 0, modelViewMatrix, 0, LIGHT_DIRECTION, 0)
        normalizeVec3(viewLightDirection)
        GLES20.glUniform4f(lightingParametersUniform, viewLightDirection[0], viewLightDirection[1], viewLightDirection[2], 1.0f)
        ...
        GLES20.glUniformMatrix4fv(modelViewUniform, 1, false, modelViewMatrix, 0)
        GLES20.glUniformMatrix4fv(modelViewProjectionUniform, 1, false, modelViewProjectionMatrixBuff)
        ...
        if(blendMode != null){
            GLES20.glDepthMask(false)
            GLES20.glEnable(GLES20.GL_BLEND)
            if(blendMode == BlendMode.Shadow){
                GLES20.glBlendFunc(GLES20.GL_ZERO, GLES20.GL_ONE_MINUS_SRC_ALPHA)
            } else if(blendMode == BlendMode.Grid){
                GLES20.glBlendFunc(GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA)
            }
        }
        GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, indexBufferId)
